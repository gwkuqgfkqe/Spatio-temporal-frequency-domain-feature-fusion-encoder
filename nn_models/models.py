...
#from utils.utils import get_adjacency_matrix, transpose_to_4d_input, init_weight_bias
...
class GraphConvolution(nn.Module):
    def __init__(self,adjacency, in_channels, out_channels, bias=True):
    ...
    def forward(self, x):
        x = torch.matmul(torch.matmul(self.adjacency, x), self.weight)
...
class ASGCNN(nn.Module):
    def __init__(self, n_channels, n_classes, input_window_size, graph_strategy='DG', kernel_length=8, bias_s=False):
        ...
        self.adjacency = get_adjacency_matrix(self.n_channels, self.graph_strategy)
        #self.register_buffer('adjacency', self.adjacency)
        ...
        self.block_gcn = GraphConvolution(self.adjacency,self.input_windows_size // 32, self.input_windows_size // 32)
...
class STGCN(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, input_window_size, kernel_length=15, drop_prob=0.2):
        ...
        adjacency = get_adjacency_matrix(self.n_channels, 'DG')
        self.block_conv = nn.Sequential(
...
class EEGNetGCN(nn.Module):
    def __init__(self, n_channels, n_classes, input_window_size,
                 F1=8, D=2, F2=16, kernel_length=32, drop_p=0.2):
        ...
        self.drop_p = drop_p
        self.input_windows_size = input_window_size
        self.adjacency = get_adjacency_matrix(self.n_channels, 'DG')
        self.block_temporal_conv = nn.Sequential(
            nn.Conv2d(1, self.F1, (1, self.kernel_length),
                      stride=1, bias=False, padding='same'),
        ...
        out = block_conv(torch.ones((1, 1, self.n_channels, self.input_windows_size), dtype=torch.float32))
        self.block_classifier = nn.Sequential(
            GraphConvolution( self.adjacency,in_channels=out.cpu().data.numpy().shape[3], out_channels=out.cpu().data.numpy().shape[3]),
            nn.BatchNorm2d(self.F2, momentum=0.01, affine=True, eps=1e-3),
            nn.ELU(),
...
class GCNEEGNet(nn.Module):
    def __init__(self, n_channels, n_classes, input_window_size,
                 F1=8, D=2, F2=16, kernel_length=64, drop_p=0.2):
...
class ASTGCN(nn.Module):
    def __init__(self, n_channels, n_classes, input_window_size, kernel_length=64, drop_p=0.2):
